...
export default function PracticeStudio({
  ...
  // --- Portrait-video mobile-only UI state (NEW, scoped) ---
  const [isVerticalVideo, setIsVerticalVideo] = useState(false);
  const [isMobile, setIsMobile] = useState(false);
  const [mobileBottomInset, setMobileBottomInset] = useState(88);

  const [isDrawerOpen, setIsDrawerOpen] = useState(false);
  const [drawerTab, setDrawerTab] = useState<"bookmarks" | "comments">(
    "bookmarks"
  );
  ...
  }, [isDrawerOpen, drawerTab, isAddingMark]);
  ...
  const MobileVerticalControls = () => (
    ...
    >
      ...
      <div className="flex items-center justify-between max-w-md mx-auto gap-2">
        {/* Mark: open drawer bookmarks + show add form */}
        <button
          onClick={() => {
            setDrawerTab("bookmarks");
